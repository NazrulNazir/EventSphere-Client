...
const recent = [
  { name: "Neon Frequencies Fest", date: "Jun 28", category: "Music", cat: "chart-1", bookings: 1248, revenue: "$42,110", status: "Live" },
  { name: "Future of Work Summit", date: "Jul 3", category: "Business", cat: "chart-2", bookings: 842, revenue: "$12,638", status: "Completed" },
  { name: "Urban Marathon Series", date: "Jul 8", category: "Sports", cat: "chart-3", bookings: 3120, revenue: "$78,400", status: "Live" },
  { name: "Modern Canvas Expo", date: "Jul 12", category: "Arts", cat: "chart-4", bookings: 512, revenue: "$8,220", status: "Scheduled" },
  { name: "AI & Robotics Expo", date: "Jul 18", category: "Technology", cat: "chart-5", bookings: 1904, revenue: "$63,600", status: "Scheduled" },
];

const upcoming = [
  { name: "AI & Robotics Expo 2026", venue: "Tech Arena, Austin", days: "8d", progress: 78 },
  { name: "Coastal Jazz Weekend", venue: "Harbor Stage, Miami", days: "14d", progress: 62 },
  { name: "Founders Retreat", venue: "Lake Tahoe Lodge", days: "21d", progress: 41 },
];

const timeline = [
  { t: "12m ago", text: "New booking on Neon Frequencies Fest (+4 tickets)", color: "chart-1" },
  { t: "38m ago", text: "Ticket tier 'VIP Lounge' sold out for AI & Robotics Expo", color: "chart-5" },
  { t: "2h ago", text: "Coastal Jazz Weekend hit 60% of capacity", color: "chart-3" },
  { t: "5h ago", text: "Refund issued for Order #48221 — Modern Canvas Expo", color: "chart-4" },
  { t: "Yesterday", text: "Urban Marathon Series broke $75K in revenue", color: "chart-2" },
];
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Friday, July 10, 2026 · <span className="text-foreground">12 active events</span> this month
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            sub={
              <span>
                This month · <span className="text-foreground">3 happening this week</span>
              </span>
            }
            tint="warning"
          />
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
          <Card className="surface-gradient col-span-1 border-border/60 p-5 lg:col-span-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h3 className="font-display text-lg font-semibold">Monthly Analytics</h3>
                <p className="text-xs text-muted-foreground">Jan — Jul 2026</p>
              </div>
              <Tabs value={range} onValueChange={setRange}>
                <TabsList className="h-9 rounded-full bg-muted/60 p-1">
                  <TabsTrigger value="revenue" className="rounded-full px-3 text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                    Revenue
                  </TabsTrigger>
                  <TabsTrigger value="bookings" className="rounded-full px-3 text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                    Bookings
                  </TabsTrigger>
                  <TabsTrigger value="events" className="rounded-full px-3 text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
                    Events
                  </TabsTrigger>
                </TabsList>
              </Tabs>
            ...
            <div className="mt-4 h-[260px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={analytics} margin={{ top: 10, right: 8, left: -12, bottom: 0 }}>
                  <defs>
          ...
          <Card className="surface-gradient col-span-1 border-border/60 p-5 lg:col-span-2">
            <div>
              <h3 className="font-display text-lg font-semibold">Event Categories</h3>
                  ...
                  <PieChart>
                    ...
                    <Tooltip
                      ...
                      formatter={((value: any, name: any) => {
                        ...
                        return Number.isNaN(percent) ? ["", name] : [`${percent}%`, name];
                      }) as any}
                    />
                  </PieChart>
                </ResponsiveContainer>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-border/60 hover:bg-transparent">
                    <TableHead className="text-[11px] uppercase tracking-wider text-muted-foreground">Event</TableHead>
                    ...
                    <TableHead className="text-right text-[11px] uppercase tracking-wider text-muted-foreground">Bookings</TableHead>
                    <TableHead className="text-right text-[11px] uppercase tracking-wider text-muted-foreground">Revenue</TableHead>
                    <TableHead className="text-[11px] uppercase tracking-wider text-muted-foreground">Status</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {recent.map((e) => (
                    <TableRow key={e.name} className="border-border/40 hover:bg-muted/30">
                      ...
                      <TableCell className="text-right text-foreground">{e.bookings.toLocaleString()}</TableCell>
                      <TableCell className="text-right text-foreground">{e.revenue}</TableCell>
                      <TableCell>
          ...
          <Card className="surface-gradient border-border/60 p-5">
            ...
            <div className="mt-4 space-y-4">
              {upcoming.map((u) => (
                <div key={u.name} className="rounded-xl border border-border/50 bg-card/60 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate font-medium text-foreground">{u.name}</p>
                      <p className="mt-0.5 text-xs text-muted-foreground">📍 {u.venue}</p>
                    </div>
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    <Progress value={u.progress} className="h-1.5 bg-muted [&>div]:bg-primary" />
...
function StatCard({
  ...
  const tintVar =
    tint === "primary" ? "var(--primary)" : tint === "warning" ? "var(--warning)" : tint === "success" ? "var(--success)" : "var(--pink)";
  return (
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[11px] font-medium ${s.bg} ${s.fg}`}>
